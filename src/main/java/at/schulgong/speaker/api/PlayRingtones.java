...
public class PlayRingtones {
    ...
    @Autowired private PlayRingtonesService playRingtonesService;
    private Timer timer;
    ...
    private void startRingtimeTask(LocalDateTime ldtPlayTime, RingtimeDTO ringtimeDTO) {
        ...
        try {
            PlayRingtoneTask ringtimeTask =
                    new PlayRingtoneTask(ringtimeDTO.getRingtoneDTO(), this);
        ...
        LocalDate endDate =
                ringtimeDTO.getEndDate() != null
                        ? ringtimeDTO
                                ...
                                .toLocalDate()
                        : null;
        if ((ld.isEqual(startDate) || ld.isAfter(startDate))
                && (endDate == null || ld.isEqual(endDate) || ld.isBefore(endDate))) {
    ...
    public boolean checkLoadHoliday(HolidayDTO holidayDTO) {
        LocalDate ld = LocalDate.now();
        return (ld.isEqual(holidayDTO.getStartDate()) || ld.isAfter(holidayDTO.getStartDate()))
     ...
     */
    public SpeakerActionStatus controlPlaylist(
            SpeakerCommandDTO speakerCommandDTO, List<PlaylistSong> playlistList) {
            ...
            if (command.equals("PLAY") || command.equals("NEXT") || command.equals("PREVIOUS")) {
                if (isPlayingFromQueue) {
                    switch (command) {
                        ...
                        case "NEXT":
                            argsListPlaylist =
                                    new String[] {SpeakerCommand.NEXT_SONG_QUEUE.getCommand()};
                            break;
                        case "PREVIOUS":
                            argsListPlaylist =
                                    new String[] {SpeakerCommand.PREVIOUS_SONG_QUEUE.getCommand()};
                            break;
                        default:
                            break;
                    }
                    ...
                    switch (command) {
                        case "PLAY":
                            isPlayingPlaylist = true;
                            argsListPlaylist =
                                    new String[] {SpeakerCommand.PLAY_FROM_QUEUE.getCommand(), "0"};
                            callPlaylistInfoThread = true;
                            break;
                        case "NEXT":
                            String position = "0";
                            ...
                            argsListPlaylist =
                                    new String[] {
                                        SpeakerCommand.PLAY_FROM_QUEUE.getCommand(), position
                                    };
                            callPlaylistInfoThread = true;
                            break;
                        case "PREVIOUS":
                            ...
                            argsListPlaylist =
                                    new String[] {
                                        SpeakerCommand.PLAY_FROM_QUEUE.getCommand(),
                                        "" + (playlistList.size() - 1)
                                    };
                            callPlaylistInfoThread = true;
                            break;
                ...
                argsListPlaylist =
                        new String[] {
                            SpeakerCommand.SET_VOLUME.getCommand(), speakerCommandDTO.getParameter()
                        };
            } else if (command.equals("MUTE")) {
                argsListPlaylist =
                        new String[] {
                            SpeakerCommand.MUTE.getCommand(), speakerCommandDTO.getParameter()
                        };
            }
    ...
    private void getPlaylistState() {
        ...
        if (speakerActionStatusPosition.getExitCode() == 0
                && speakerActionStatusPosition.getException() == null) {
            position = speakerActionStatusPosition.getInformation();
        }
        String[] argsPlaylistPosition =
                new String[] {SpeakerCommand.GET_PLAYLIST_POSITION.getCommand()};
        SpeakerActionStatus speakerActionStatusPlaylistPosition =
                executeSpeakerAction(argsPlaylistPosition);
        if (speakerActionStatusPlaylistPosition.getExitCode() == 0
                && speakerActionStatusPlaylistPosition.getException() == null) {
            int playlistPositionNumber =
                    Integer.parseInt(speakerActionStatusPlaylistPosition.getInformation()) - 1;
        ...
        try {
            ...
            if (speakerActionStatus.getInformation() != null) {
                ...
                playlistInfo =
                        objectMapper.readValue(
                                speakerActionStatus.getInformation(), PlaylistInfo.class);
                if (playlistInfo.isPlayingFromQueue()
                        && playlistInfo.getPosition() == 1
                        && playlistInfo.getSpeakerState().equals(SpeakerState.STOPPED.getState())) {
                    if (repeatPlaylist) {
                        String[] args = {SpeakerCommand.PLAY.getCommand()};
                        SpeakerActionStatus speakerActionStatusRepeat = executeSpeakerAction(args);
                        if (speakerActionStatusRepeat.getExitCode() == 0
                                && speakerActionStatusRepeat.getException() == null) {
                ...
                if (!playlistSongDTOList.isEmpty()) {
                    actualPlaylistSongDTO =
                            playlistSongDTOList.get(
                                    playlistInfo.getPosition() == 0
                                            ? 0
                                            : playlistInfo.getPosition() - 1);
                } else {
                    isPlayingPlaylist = false;
            ...
            playlistDTO =
                    PlaylistDTO.builder()
                            ...
                            .songDTOList(playlistSongDTOList)
                            .build();

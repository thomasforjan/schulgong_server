...
public class SongModelAssembler extends RepresentationModelAssemblerSupport<Song, SongDTO> {
  ...
  public SongDTO toModel(Song song) {
    ...
    songDTO.add(linkTo(methodOn(LiveController.class).allSongs()).withRel(Config.SONG.getUrl()));
    return songDTO;
  }
  ...
  public CollectionModel<SongDTO> toCollectionModel(Iterable<? extends Song> songList) {
    ...
    for (Song song : songList) {
      SongDTO songDTO = DtoConverter.convertSongToDTO(song);
      songDTO.add(linkTo(methodOn(HolidayController.class).one(songDTO.getId())).withSelfRel());
      songDTO.add(linkTo(methodOn(HolidayController.class).all()).withRel(Config.SONG.getUrl()));

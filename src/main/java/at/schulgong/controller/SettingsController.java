        ...
        if (configurationDTO != null) {
            ...
            try {
                ReadWriteConfigurationFile.writeConfigurationDTOFromConfigFile(
                        Config.CONFIGURATION_PATH.getPath(), configurationDTO);
    ...
    public ResponseEntity updatePassword(@RequestBody Map<String, String> payload) {
        ...
        if (configurationDTO != null
                && !argon2PasswordEncoder.matches(
                        currentPassword, configurationDTO.getPassword())) {
        ...
        if (!newPassword.equals(confirmNewPassword)) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Passwörter stimmen nicht überein!");
        }

        // Check if the new password is at least 6 characters long
        if (newPassword.length() < 6) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Passwort muss mindestens 6 Zeichen lang sein.");
        }

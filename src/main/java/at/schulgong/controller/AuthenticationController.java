    ...
    public ResponseEntity<?> authenticate(@RequestBody LoginDTO loginDTO) {
        ...
        if (configurationDTO == null) {
            return ResponseEntity.status(500)
                    .body(
                            "Server-Konfigurationsdatei konnte nicht gelesen werden. Bitte"
                                    + " kontaktieren Sie den Systemadministrator.");
        }

        boolean isPasswordMatch =
                argon2PasswordEncoder.matches(
                        loginDTO.getPassword(), configurationDTO.getPassword());
        ...
        if (isPasswordMatch) {
            ...
            return ResponseEntity.status(401)
                    .body("Fehlerhafte Anmeldung. Bitte überprüfen Sie Ihr Passwort.");
        }
    }

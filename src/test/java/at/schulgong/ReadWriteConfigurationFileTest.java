...
class ReadWriteConfigurationFileTest {
  ...
  public void testWriteConfigurationDTOFromConfigFile() throws IOException {
    ...
    ReadWriteConfigurationFile.writeConfigurationDTOFromConfigFile(CONFIGURATION_FILE_PATH,
      dummyWriteConfigurationDTO);

    ObjectMapper objectMapper = new ObjectMapper();
    objectMapper.registerModule(new JavaTimeModule());
    Path path = Paths.get(CONFIGURATION_FILE_PATH);
    ConfigurationDTO configurationDTO = objectMapper.readValue(new File(path.toAbsolutePath().toUri()), ConfigurationDTO.class);

    assertEquals(dummyWriteConfigurationDTO.getPassword(),
      configurationDTO.getPassword());
    assertEquals(dummyWriteConfigurationDTO.getRingtimeVolume(),
      configurationDTO.getRingtimeVolume());
    assertEquals(dummyWriteConfigurationDTO.getAlarmVolume(),
      configurationDTO.getAlarmVolume());
    assertEquals(dummyWriteConfigurationDTO.getAnnouncementVolume(),
      configurationDTO.getAnnouncementVolume());
    assertEquals(dummyWriteConfigurationDTO.getRingtimeDirectory(),
      configurationDTO.getRingtimeDirectory());
    assertEquals(dummyWriteConfigurationDTO.getPlaylistDirectory(),
      configurationDTO.getPlaylistDirectory());
  }
}
